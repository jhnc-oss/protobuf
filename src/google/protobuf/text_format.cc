...
#undef SET_FIELD
  ...
  // If `prefix_required` is true, a full type name without a type URL prefix is
  // rejected, i.e., only full type URLs are accepted.
  // The full type name is returned in `full_type_name`, and the type URL prefix
  // is returned in `prefix` (which is empty if a full type name without a
  // prefix is consumed).
  bool ConsumeAnyTypeUrlOrFullTypeName(std::string* full_type_name,
    ...
    if (last_slash_pos == -1) {
      ...
      *prefix = "";
      // Further validation of the type name is done below.
      *full_type_name = text;
    } else {
